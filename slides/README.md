...
Beamer source for the live talk, in progress. The slide-level content plan is [storyboard.md](storyboard.md) (supersedes `outline.md`'s section list); visual sourcing per slide is [image_plan.md](image_plan.md); typography/color/density/logo rules are [style_guide.md](style_guide.md), extracted from Alex's own real PowerPoint talks. Prologue and Act I are built (`00_open.tex`, `00b_roadmap.tex`, `01_ecosystem.tex`, `02_act1_understand.tex`); Act II, Act III, and the Epilogue don't have section files yet — see `storyboard.md`'s status line.
