...
\begin{frame}{Task 4 --- baseline before touching anything}
  \vfill
  \begin{center}
    Refactoring code behind a published result, with no safety net, means you can never prove the answer didn't move.
  \end{center}
  \vspace{0.6em}
  \begin{center}
    The gotcha: pin the \textit{original} code's exact output as a regression test \textbf{before} touching a single line of the new structure --- not partway through, once refactoring has already started.
  \end{center}
  \vspace{0.6em}
  \begin{center}
    \slidepoint{Every future change either matches the pinned value, or it doesn't --- no ``looks about right.''}
  \end{center}
  \vfill
\end{frame}
...
\begin{frame}{Task 3, continued --- one manuscript, three places, one history}
  ...
  \vspace{-0.3cm}
  \begin{center}\small
    Back to Task 3's proposal --- the same discipline that made Task 4 safe applies just as well to a document.
  \end{center}
  \vspace{0.3em}
  \begin{center}
