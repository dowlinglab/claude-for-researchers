...
\begin{frame}{The filesystem is the source of truth}
  \begin{center}
   ...
   % TODO: Help me reduce the white space betewen the top of the columns and the bottom of the line above.
  \end{center}
  \begin{columns}[t]
  ...
  \vspace{0.2em} % Can we move these linked URLs to numbered references at the bottom of the slide? Right now, it is not clear these are hyperlinks. 
    Getting started is easy: create a GitHub account, apply for \href{https://github.com/education}{education free upgrades}, watch a \href{https://youtu.be/a9u2yZvsqHA?si=Tose1lV36yNAaf-q}{YouTube tutorial}, install \href{https://desktop.github.com/download/}{GitHub Desktop}.
  \end{center}
\end{frame}

\begin{frame}{An agent can touch your repository --- a chatbot can't}
  \vfill
  \begin{center} % TODO: Make the text larger in this graphic.
  \begin{tikzpicture}[font=\small, node distance=1.1cm]
    ...
    \node[draw=NDGold, rounded corners=2pt, minimum width=3.6cm, minimum height=1.3cm, align=center, right=of b, fill=CodeBg]
      (c) {\textbf{Agent}\\\scriptsize LLM + tools\\\scriptsize reads, writes, and executes your files};
...
\begin{frame}{Task 3 --- idea first: brainstorm, refine, then bring in the literature}
  ...
  \end{center}
  \vspace{0.5em}
  \begin{center} % TODO Let's make these numbered steps for performing a corpus-grounded literature review. One of the steps will be downloading papers.
  \begin{itemize}\small\setlength\itemsep{0.25em}
    \item Search a second, adjacent field on purpose --- not just your own topic
    \item Process papers in small batches, each with a checkpoint before the next starts
