...
\begin{frame}{A repository beats a folder, and a folder beats a chat}
  ...
  \begin{columns}[t]
    ...
    \end{column}
    \begin{column}{0.30\textwidth}
      \begin{center}\textbf{A file system}\end{center}
      \begin{itemize}\footnotesize\setlength\itemsep{0.22em}
    ...
    \end{column}
    \begin{column}{0.30\textwidth}
      \begin{center}\textbf{\color{IrishGreen}A repository}\end{center}
      \begin{itemize}\footnotesize\setlength\itemsep{0.22em}
...
\begin{frame}[fragile]{Ask for help writing the goal, not just the work}
  ...
  \vspace{0.35cm}
  \begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
I would like help developing a goal for an overnight session.

The work: set up the inherited project so I can reproduce Figure 3
of the paper.

Draft a goal document that states:
- the objective, and the evidence that proves it is finished
- which files you may change, and which you may not
- phases, each ending in a check I can verify myself
- what to do when a check fails
- the decisions you must leave to me
- a commit after each phase, and a morning review queue

Ask me questions before you write it.
  \end{lstlisting}
  \vspace{0.3em}
  \begin{center}
    \slidepoint{The first overnight prompt is about the \textbf{goal}, not the work.}
  \end{center}
  \vspace{0.5cm plus 1fill}
\end{frame}
...
\begin{frame}{Then draft, review, and launch}
  ...
  \begin{enumerate}\setlength\itemsep{0.75em}
    ...
    \item \textbf{Start the run} with the reviewed document as the goal:
