...
\begin{frame}{Task 3 --- idea first: brainstorm, refine, then bring in the literature}
  ...
  \begin{enumerate}\small\setlength\itemsep{0.4em}
    \item Search for literature in adjacent communities/disciplines, not just your subarea
    \item Process PDFs in small topical batches, with a checkpoint after each batch
    \item Propose consistent PDF names; flag probable duplicates, never delete them automatically
    \item Maintain \texttt{ref.bib} and summarize each paper in \texttt{literature.md}
    \item Verify relevance, metadata, and quantitative claims against the source PDFs
  \end{enumerate}
  \end{center}
  \vspace{1.8cm plus 1fill}
\end{frame}
