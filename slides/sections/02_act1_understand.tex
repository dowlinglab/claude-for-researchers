...
\begin{frame}{An agent can touch your repository --- a chatbot can't}
  ...
  \vspace{0.6em}
  \begin{center}
    A chatbot only has the conversation and the files you share (e.g., uploading or attaching a PDF).
  \end{center}
  \vspace{0.6em}
  \begin{center}
...
\begin{frame}{Task 2 --- a chat history can't hold any of these three}
  ...
  \begin{columns}[t]
    \begin{column}{0.31\textwidth}
      ...
      Your data and results don't change between sessions --- a changed number is a finding, not noise from a different chat.\\[0.7em]
      {\scriptsize$\rightarrow$ true across every session}
    \end{column}
    \begin{column}{0.31\textwidth}
      {\color{NDBlue}\textbf{Authoritative}}\\[0.4em]
      The actual code and data --- point an agent at the real files, never describe them from memory.\\[0.7em]
      {\scriptsize$\rightarrow$ point at it, never summarize it}
    \end{column}
    \begin{column}{0.31\textwidth}
      {\color{NDBlue}\textbf{Task}}\\[0.4em]
      What you're working on right now, defined fresh each session.\\[0.7em]
...
## Current state
  ...
  Resume at: notes/log.md, 2026-08-14
    ...
    \begin{column}{0.34\textwidth}
      \small
      Name it neutrally: \texttt{CLAUDE.md} (Claude Code) or \texttt{AGENTS.md} (Codex, Cursor, Gemini CLI, and 20+ other tools) --- both become three-line pointers to it.\\[0.6em]
      Neither tool reads the other's file --- so the content lives
      in one place and the tool files just point.\\[0.6em]
