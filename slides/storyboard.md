...
|---|---|---|---|---|
| — | Title | | 1 | 0.5 min |
| Prologue | **Tinker** | Hook in full (real dates, visuals), the claim, scope, orient, show the toolkit and the archetypes, prove the thesis is live | 9 | 8 min |
...
| | | **Total** | **44** | **~48 min**, trim ~8 in drafting/rehearsal |
...
**Revised 2026-09-03** at Alex's request: the single-slide teaser undersold the hook, so it's now two slides with real detail and visuals, and "the claim" moved from opener to takeaway (was items 2/3 pre-2026-09-02; before that, items 2 and 5 were cut and restored on 2026-09-02 — see the open-questions entry below for that history). Two IT-policy slides (items 7-8, below) moved to the Epilogue on Alex's suggestion. Net Prologue slide count is unchanged at 9 (two new hook slides and a new archetypes slide, offset by the two moved-out policy slides).
...
6. **One year of practices, not a tool tutorial** — *(built, `01_ecosystem.tex`)* scope the talk: not a Claude tutorial, not a product comparison, not a claim that AI does research; instead one year of practices and their failures, mostly model-independent, things to change on Monday. Closes with the act-strip (still at 0 — nothing lit yet).
7. **Don't become loyal to a model** — *(built)* pick tools by task, not brand. Kept in the Prologue (not moved to the Epilogue with items 8-9 below) on Alex's own argument: it directly cashes out item 6's "not a product comparison" claim.
...
- **Slide count vs. budget.** Alex confirmed 2026-09-02: a 60-minute slot, targeting 35-40 slides to leave real room for Q&A and discussion. Current total is 44 — over that range, after the 2026-09-03 revision added four `\stagedivider` transition slides (quick and sparse, not full content beats — Alex's own request) and a fuller Prologue hook, partly offset by cutting Act I's opening slide and merging Act II's old two-slide opener into one. The at-a-glance table's rough-time estimates (~48 min) still run over a 40-minute talk portion. Deliberately not cutting further right now; a rehearsal pass with real timing, not a slide-count target, should drive any further trim — the dividers in particular should read as very fast in person even though they count as slides.
