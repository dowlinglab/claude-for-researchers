...
- **The legend does not cover any plotted data.** This is a correctness issue, not a matter of taste: a legend sitting on the curves hides the evidence the figure exists to show. `loc="best"` minimises overlap but does not eliminate it, and it gives up silently when the axes are full. Fixes, in order of preference: anchor the legend outside the axes; extend the axis limits to open up space; reduce the number of series; or label series directly and drop the legend. This one is checkable automatically — see [`../scripts/figure_style/`](../scripts/figure_style/).
- Nothing else overlaps either: annotations, inset axes, and error bars at the axis limits.
...
- **A legend parked on top of the curves.** Usually `loc="best"` on a full plot, and usually invisible to the person who made it because they already know what the data looks like.
