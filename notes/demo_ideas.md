...
| 10 | Task 7 — a week before the semester, at scale | Type 6, named — `optimization`/`optimization-private`: ~1,450 commits in 2.5 weeks, 98% of the private repo's entire history. **Rebuilt 2026-09-04 (revision #12):** opens with a six-item numbered list of everything in scope (mirrors the Task 4 recipe slide), so rows 11-15 below can each elaborate selectively | The same shape as the Prologue's hook — years of patches, then a burst | course site: `ndcbe.github.io/optimization` |
...
| 6 | Task 8 (hobbies), the final example | boot-dryer-octopus; kitchen organizer/telescope; `radio-extra-book`. **Revision #12:** the 3D-printing workflow grew a sketch/photo step at the front and an explicit feedback-loop step at the end, per Alex's own first-person account; both hobby figures enlarged | The same discipline pays off outside the lab too |
| 7 | So, what now? (final slide) | Generic — the concrete six-step call to action; cites the new `resources/prompts/getting_started.md`. **"One thing to change tomorrow morning" cut 2026-09-04 (revision #12)** — its message duplicated Task 2, and its GitHub-repo pointer line is now folded into this slide instead | The resources are what your chatbot builds on, not a tutorial to finish |
