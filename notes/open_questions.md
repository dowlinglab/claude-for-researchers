...
5. **Capture the desktop app screenshot for slide 8.** Slides 9 and 10 now use official Claude
   Code documentation screenshots, but **the documentation has no image of the desktop app**, so
   slide 8 still holds a placeholder. It has to come from Alex's own setup: the Code tab with the
...
6. **Re-time the talk.** The deck is 55 slides against a 40-minute target that was rehearsed at 52.
   The three workspace slides (8-10) are the obvious candidates to re-merge if rehearsal runs long.
